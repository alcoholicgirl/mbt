    ...
    T: 'static + Send + Sync,
{
    fn spawn(
        ...
        // Store the NSView so the plugin can drive NSCursor directly (baseview
        // doesn't support cursor changes on macOS).
        #[cfg(target_os = "macos")]
        if let ParentWindowHandle::AppKitNsView(ns_view) = &parent {
            egui_state
                .ns_view
                .store(*ns_view as usize, Ordering::Relaxed);
        }
            ...
            move |egui_ctx, queue, state| {
                ...
                if let Some(new_size) = egui_state.requested_size.swap(None) {
                    ...
                    if context.request_resize() {
                        // Resize the content of egui window. `new_size` is in logical points, but
                        // `PhySize` expects physical pixels, so it needs to be scaled by the
                        // current scale factor. Otherwise the framebuffer is sized wrong on HiDPI
                        // displays (e.g. Retina), which makes the content flicker/clip while
                        // dragging the resize corner.
                        let scale = egui_ctx.pixels_per_point() as f64;
                        queue.resize(PhySize::new(
                            (new_size.0 as f64 * scale).round() as u32,
                            (new_size.1 as f64 * scale).round() as u32,
                        ));
