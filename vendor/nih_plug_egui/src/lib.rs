...
pub struct EguiState {
    ...
    /// The AppKit `NSView` pointer for the editor (macOS only). Used to work
    /// around baseview not supporting cursor changes on macOS.
    #[serde(skip)]
    pub ns_view: AtomicUsize,
}
